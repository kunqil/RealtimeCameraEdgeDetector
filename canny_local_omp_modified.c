...
/*
  Guidelines for compiling and running the program are given below!
*/
...
/* These are the function declerations so that the compiler knows what functions,
we will be using in the program, before the program even starts.
...
int main(int argc, char *argv[])
{
   ...
   ****************************************************************************/
   /*These are the inputs we are providing the program while using it.
    This program expects 4 different arguments. infilename, sigma, tlow and thigh.
    While running the program in the command line you should run it as the following:

    ```
    gcc canny_local_omp.c -lm -fopenmp -o ./canny_omp

    ./canny_omp test.pgm 1.0 1.0 0.8
    ```

    test.pgm is the infilename
    1.0 -> sigma
    Second 1.0 -> tlow
    0.8 -> thigh
   */
   ...
   ****************************************************************************/
   ...
   //This canny function does the whole thing.
...
//EDITTING BEGINS
   ...
   ****************************************************************************/
   ...
   int n_thread = 4;					//Edited for multi-threading
   omp_set_dynamic(0);					//Edited for multi-threading
   omp_set_num_threads(n_thread);			//Edited for multi-threading
   #pragma omp parallel private(c, cc, dot, sum)	//Edited for multi-threading
   {
      printf("X Direction Thread ID = %d\n", omp_get_thread_num()); // Edited for multi-thread checking
   ...
   ****************************************************************************/
   omp_set_dynamic(0);					//Edited for multi-threading
   omp_set_num_threads(n_thread);			//Edited for multi-threading
   #pragma omp parallel private(r, rr, dot, sum)	//Edited for multi-threading
   {
     printf("Y Direction Thread ID = %d\n", omp_get_thread_num()); //Edited to check multi-threading
